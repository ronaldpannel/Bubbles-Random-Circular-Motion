...
let particleArray = [];
let numOfParticles = canvas.width * 0.6;
...
class PushRing {
  ...
  update() {
    ...
    this.y += Math.cos(this.angle) * this.curve;
      ...
      if (
        pushRing.x + pushRing.radius > canvas.width ||
        pushRing.x < pushRing.radius
      ) {
        location.reload();
      }

      if (
        pushRing.y + pushRing.radius > canvas.height ||
        pushRing.y < pushRing.radius
      ) {
        location.reload();
      }
  }
